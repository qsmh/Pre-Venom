...
public class DamageManager implements Listener {
    ...
    public double getDamageFromEvent(EntityDamageByEntityEvent event) {
        double damage = event.getDamage() * (Double)this.additiveDamageBuffer.getOrDefault(event, 1.0) * (Double)this.multiplicativeDamageBuffer.getOrDefault(event, 1.0) * (Double)this.reductionBuffer.getOrDefault(event, 1.0) - (Double)this.absoluteReductionBuffer.getOrDefault(event, 0.0);
    ...
    public double getFinalDamageFromEvent(EntityDamageByEntityEvent event) {
        double damage = event.getFinalDamage() * (Double)this.additiveDamageBuffer.getOrDefault(event, 1.0) * (Double)this.multiplicativeDamageBuffer.getOrDefault(event, 1.0) * (Double)this.reductionBuffer.getOrDefault(event, 1.0) - (Double)this.absoluteReductionBuffer.getOrDefault(event, 0.0);
        if (this.removeCriticalDamage.contains(event)) {
            damage *= 0.667;
        }
    ...
    public void addDamage(EntityDamageByEntityEvent event, double value, CalculationMode mode) {
        if (mode == CalculationMode.ADDITIVE) {
            this.additiveDamageBuffer.put(event, (Double)this.additiveDamageBuffer.getOrDefault(event, 1.0) + value);
        }

        if (mode == CalculationMode.MULTIPLICATIVE) {
            this.multiplicativeDamageBuffer.put(event, (Double)this.multiplicativeDamageBuffer.getOrDefault(event, 0.0) + value);
        }
    ...
    public void reduceDamage(EntityDamageByEntityEvent event, double value) {
        ...
        if ((Double)this.reductionBuffer.get(event) == 1.0) {
            this.reductionBuffer.put(event, (Double)this.reductionBuffer.get(event) - (1.0 - value));
        } else {
            this.reductionBuffer.put(event, 1.0 - (Double)this.reductionBuffer.get(event) * value);
        }
    }
